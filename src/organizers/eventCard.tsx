...
import {
  ...
  createVolunteerStatistics,
  decrementEventVolunteerCount,
} from "./api";
...
const VolunteerStatsModal = ({
  ...
  const validateStats = () => {
    ...
    if (stats.minutes_worked <= 0) {
      newErrors.minutes_worked =
        "Išdirbtas laikas turi būti teigiamas skaičius";
      isValid = false;
    }
    ...
    <Modal open={open} onClose={onClose} aria-labelledby="stats-modal-title">
      ...
      >
        ...
        <TextField
          ...
          value={stats.rating}
          onChange={(e) =>
            setStats({ ...stats, rating: parseFloat(e.target.value) })
          }
        ...
        <TextField
          ...
          onChange={(e) =>
            setStats({ ...stats, minutes_worked: parseInt(e.target.value) })
          }
          inputProps={{ min: 0 }}
...
const EventCard = ({ event }: { event: Event }) => {
  ...
  const [selectedVolunteerForStats, setSelectedVolunteerForStats] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const notCancelledApplications = applications.filter(
          (application) => application.status !== "atmesta"
        );
        setVolunteerRequests(notCancelledApplications);

        const accepted = applications.filter(
          (application) => application.status === "priimta"
        );
  ...
  const filteredVolunteers = volunteerRequests?.filter(
    (volunteer) =>
    ...
    <>
      ...
      <Card sx={{ marginBottom: 2 }}>
        <CardContent>
          ...
          <List>
            {volunteerRequests
              ?.filter((application) => application.status === "priimta")
              .map((application) => (
                <ListItem
                ...
                >
                  ...
                  <Button
                    ...
                    size="small"
                    onClick={() =>
                      handleOpenStatsModal(application.volunteer.id)
                    }
                    sx={{ ml: 2 }}
                  >
                    Pridėti statistiką
